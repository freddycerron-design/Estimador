...
function confidenceBadge(score: string | null): { label: string; className: string } {
  if (score === null) return { label: "—", className: "bg-slate-100 text-slate-500 dark:bg-navy-700 dark:text-slate-400" };
  const pct = Math.round(Number(score) * 100);
  if (pct >= 75) return { label: `${pct}% Alto`, className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" };
  if (pct >= 50) return { label: `${pct}% Medio`, className: "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300" };
  return { label: `${pct}% Bajo`, className: "bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300" };
}
      ...
      <div className={`${card} overflow-hidden`}>
            ...
            {estimates.map((e) => {
              ...
              return (
                <tr key={e.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-navy-700 dark:hover:bg-navy-700/40">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-400 dark:text-slate-500">{new Date(e.created_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1 font-medium text-slate-500 hover:text-brand-600 hover:underline dark:text-slate-400 dark:hover:text-brand-400"
        ...
        {estimates.length === 0 && !error && (
          <p className="p-6 text-center text-sm text-slate-400 dark:text-slate-500">
            Aún no hay estimaciones. Empieza una desde un requerimiento registrado, en la sección Requerimientos.
          </p>
        )}
      </div>
    </div>
